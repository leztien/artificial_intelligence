...
"""
best first search
very close to the pseudo-code in AIMA on page 91
"""
...
class PriorityQueue:
    ...
    def __init__(self, f=None):
        self._nodes = []
        self.f = f if callable(f) else lambda x: x
...
class AbstractSearchProblem(ABC):
    ...
    @abstractmethod
    def action_cost(state, action, new_state) -> float:
        "returns the cost of performing an action from the current state that takes the agent to the new_state"
...
class SearchProblem(AbstractSearchProblem):
    ...
    def action_cost(self, state, action, new_state) -> float:
        assert action.go_to_state is new_state, "must be able to get to the new state with the action provided"
        i,j = state.id, new_state.id
...
############################################################################
### helper utilities e.g. random problem generation
...
if __name__ == '__main__':
    ...
    seeds = [7763, 6907, 2057, 685, 8169, 6577, 3577, 3641, True]
    ...
    for seed in seeds:
        ...
        output = best_first_search(problem, f)
